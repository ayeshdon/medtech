...
public class JobCardDBAccess {
	...
	private class databaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			try {
				...
				db.execSQL("CREATE TABLE " + ENGINEER_HIS + "(" + KEY_ID
						+ " INTEGER PRIMARY KEY AUTOINCREMENT, " + JOBCARDID
						+ " TEXT  ," + ENGINEERID + " TEXT  ," + DATE
						+ " TEXT  ," + JOBSTART + " TEXT  ," + JOBFINISHED
						+ " TEXT  ," + TIMEWORK + " TEXT  ," + TRAVELTIME
				...
				db.execSQL("CREATE TABLE " + PART_TB + "(" + KEY_ID
						+ " INTEGER PRIMARY KEY AUTOINCREMENT, " + PART_NAME
						+ " TEXT  ," + PARTID + " TEXT  ," + JOBCARDID
				...
				db.execSQL("CREATE TABLE " + TABLE_NAME + "(" + KEY_ID
						+ " INTEGER PRIMARY KEY AUTOINCREMENT, " + ENGINEER
						+ " TEXT ," + DATECREATED + " TEXT ," + CUSTOMER
						+ " TEXT , " + EQUIPMENT + " TEXT , " + ORDERNO
						+ " TEXT , " + VISITREASON + " TEXT , "
						+ SERVICEDETAILS + " TEXT , " + PARTSUSED + " TEXT , "
						+ FILAMENTCOUNTER + " TEXT , " + HVCOUNTER + " TEXT , "
						+ LOGTIME + " TEXT , " + DOWNTIME + " TEXT , "
						+ CUSSIGN + " TEXT , " + ENGSIGN + " TEXT , "
						+ INSPECTION + " TEXT , " + VISUAL + " TEXT , "
						+ STATUS + " INTEGER , " + VAL + " INTEGER , "
				...
				db.execSQL("CREATE TABLE " + ITEMS + "(" + KEY_ID
						+ " INTEGER PRIMARY KEY AUTOINCREMENT, " + EQUIPMENTID
						+ " TEXT  , " + EQUIPMENTTYPE + " TEXT  , " + CUSTOMER
						+ " TEXT  , " + MANUFACTURER + " TEXT  , " + MODELNO
				...
				db.execSQL("CREATE TABLE " + CUSTOMER_TB + "(" + KEY_ID
						+ " INTEGER PRIMARY KEY AUTOINCREMENT, " + CUSTOMERID
						+ " TEXT  , " + NAME + " TEXT  , " + CUSCONTACTNO
				...
				db.execSQL("CREATE TABLE " + ENGINEER_TB + "(" + KEY_ID
						+ " INTEGER PRIMARY KEY AUTOINCREMENT, " + ENGINEERID
	...
	public ArrayList<UnfinishedDataBean> getAll(int status) {
		...
		try {
			...
			String[] colums = new String[] { KEY_ID, ORDERNO, CUSTOMER,
					LASTUPDATEDATE, VAL };

			Cursor cus = DB.query(TABLE_NAME, colums, STATUS + " = " + status,
					null, null, null, null);
			...
			for (cus.moveToFirst(); !cus.isAfterLast(); cus.moveToNext()) {
				...
				for (int i = 0; i < VarList.CUSTOMER_LIST.size(); i++) {
					...
					if (VarList.CUSTOMER_LIST.get(i).getCus_ID()
							.equals(cus.getString(rowName))) {
	...
	public JobCardDataBean getJobcardDetials(long l, Boolean flag) {
		try {
			...
			String[] colums = new String[] { KEY_ID, ENGINEER, DATECREATED,
					CUSTOMER, EQUIPMENT, ORDERNO, VISITREASON, SERVICEDETAILS,
					PARTSUSED, FILAMENTCOUNTER, HVCOUNTER, LOGTIME, DOWNTIME };

			JobCardDataBean bean = new JobCardDataBean();

			Cursor cus = DB.query(TABLE_NAME, colums, KEY_ID + " = " + l, null,
					null, null, null);
			...
			if (cus != null) {
				...
				// ------------------------------part get
				// ----------------------------------------
				String[] columsPart = new String[] { KEY_ID, PART_NAME,
						QUANTITY, PARTID };

				Cursor cusPart = DB.query(PART_TB, columsPart, JOBCARDID
				...
				for (cusPart.moveToFirst(); !cusPart.isAfterLast(); cusPart
						.moveToNext()) {
					...
					partBean.setPartQuantitys(cusPart
							.getString(partQuantityIndex));
				...
				if (flag) {
					...
					String[] columsENG = new String[] { KEY_ID, ENGINEERID,
							DATE, JOBSTART, JOBFINISHED, TIMEWORK, TRAVELTIME,
							ENGSIGN };

					Cursor cusENG = DB.query(ENGINEER_HIS, columsENG, JOBCARDID
							+ " = " + l, null, null, null, null);
					...
					for (cusENG.moveToFirst(); !cusENG.isAfterLast(); cusENG
							.moveToNext()) {
						...
						for (int i = 0; i < VarList.ENGINEER_LIST.size(); i++) {
							...
							if (VarList.ENGINEER_LIST.get(i).getID()
									.equals(cusENG.getString(engID))) {

								engName = VarList.ENGINEER_LIST.get(i)
										.getEngName();
								break;
							}
						}
	...
	public void updateFirstDB(JobCardDataBean bean, long val) {
		try {
			...
			// bean.getCustomerID()!=null) {
			cv.put(CUSTOMER, bean.getCustomerID());
			// }

			// if (!bean.getEqucmentID().equals("") ||
			// bean.getEqucmentID()!=null) {
			cv.put(EQUIPMENT, bean.getEqucmentID());
			// }
	...
	public void updateFull(JobCardDataBean bean, long val) {
		...
		for (int j = 0; j < dataBeanList.size(); j++) {
			...
			if (data.getFlag().equals("0")) {
				DB.update(ENGINEER_HIS, cv1, KEY_ID + " = " + data.getID(),
						null);
			} else {
				DB.insert(ENGINEER_HIS, null, cv1);
	...
	public ArrayList<CustomerBean> getAllCustomer() {
		...
		try {
			...
			Cursor cus = DB.query(CUSTOMER_TB, colums, null, null, null, null,
					null);
	...
	public ArrayList<EngineerGetBean> getAllEngineer() {
		...
		try {
			...
			Cursor cus = DB.query(ENGINEER_TB, colums, null, null, null, null,
					null);
	...
	public JobCardDataBean getAllDetails(String jobCardID) {
		...
		try {
			...
			String[] colums = new String[] { KEY_ID, ENGINEER, DATECREATED,
					CUSTOMER, EQUIPMENT, ORDERNO, VISITREASON, SERVICEDETAILS,
					PARTSUSED, FILAMENTCOUNTER, HVCOUNTER, LOGTIME, DOWNTIME,
					CUSSIGN, ENGSIGN, LASTUPDATEDATE, INSPECTION, VISUAL };
			...
			String[] columsPart = new String[] { KEY_ID, PART_NAME, QUANTITY,
					PARTID };
			...
			String[] columsENG = new String[] { KEY_ID, ENGINEERID, DATE,
					JOBSTART, JOBFINISHED, TIMEWORK, TRAVELTIME, ENGSIGN };
