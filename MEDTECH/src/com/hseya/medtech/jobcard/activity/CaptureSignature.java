...
public class CaptureSignature extends Activity implements OnClickListener {
	...
	private Button mClear, mGetSign, mCancel,viewPartsBtn,viewEngBtn,viewJobCard;
	...
	public void onCreate(Bundle savedInstanceState) 
	{
		...
		uniqueId = getTodaysDate() + "_" + getCurrentTime() + "_" + Math.random()+"_"+VarList.USER_NAME+"CUS";
		//		engUniqueId =getTodaysDate() + "_" + getCurrentTime() + "_" + Math.random()+"_"+VarList.USER_NAME+"ENG";
		...
		//		engContent = (LinearLayout) findViewById(R.id.egineerSignaturelayout);
			...
			public void onClick(View arg0) {
				...
				if (VarList.SELETCED_BTN.equals(ConstList.CREATE_JOBCARD)) {
					try {
						...
						//save data
						JobCardDBAccess dbSQl=new JobCardDBAccess(CaptureSignature.this);
						dbSQl.openDB();
						...
						if (isItwork) {
							...
							Toast.makeText(CaptureSignature.this,MessageClass.JOBCARD_UPLOAD_LOCAL_PART_SUCCESS ,Toast.LENGTH_LONG).show();

							VarList.JOB_BEAN=null;


							Intent call2=new Intent(CaptureSignature.this,MianMenuActivity.class);
							startActivity(call2);
							CaptureSignature.this.finish();

						} else {

							Toast.makeText(CaptureSignature.this,MessageClass.JOBCARD_FIAL_LOCAL_SUCCESS ,Toast.LENGTH_LONG).show();
		...
		{        
...

			{
				...
				try {
					...
					//					EditText emailContent=(EditText) findViewById(R.id.emailDetailsTxt);
					//					String email=emailContent.getText().toString();
					boolean flag=true;
					if (!sendMail.equals("")) {
						//Toast.makeText(CaptureSignature.this,MessageClass.SENDEMAIL_EMPTY ,Toast.LENGTH_LONG).show();

						if(!ValidateClass.isValidEmailAddress(sendMail)){
							flag=false; 
							Toast.makeText(CaptureSignature.this,MessageClass.SENDEMAIL_INVALID ,Toast.LENGTH_LONG).show();
						}else{
							flag=true;
						}
						//					}else if(!ValidateClass.isValidEmailAddress(sendMail)){ 

						//					}else if(email.equals("")){ 
						//						Toast.makeText(CaptureSignature.this,MessageClass.EMAILCON_EMPTY ,Toast.LENGTH_LONG).show();
					}else{
					...
					e.printStackTrace();
					Toast.makeText(CaptureSignature.this,"Unknown Error form Server " ,Toast.LENGTH_LONG).show();
				}
			}
	 ...
	 */
	private void updateToLocalDataBase() {
		...
		.setMessage("No internet connection. Data save to local database")
		...
		public void save(View v,String check) 
		{
			...
			if (check.equals("C")) {
				...
				try
				{
					...
					//	Images.Media.insertImage(getContentResolver(), mBitmap, "title", null);
	...
	public void onBackPressed() {
		...
		.setPositiveButton("Go to home",new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog,int id) {
				...
				Intent call=new Intent(CaptureSignature.this,MianMenuActivity.class);
				startActivity(call);
				CaptureSignature.this.finish();
	...
	public void saveContact(){
		...
		try {
			...
			if (VarList.SELETCED_BTN.equals(ConstList.UNFINISHED_JOBCARD)) { 
				dbClass.updateFull(VarList.JOB_BEAN, Long.parseLong(VarList.SELECTED_ID));
			} else{
			...
			if (isItwork) {
				...
				Toast.makeText(CaptureSignature.this,MessageClass.JOBCARD_UPLOAD_LOCAL_SUCCESS ,Toast.LENGTH_LONG).show();
	...
	private class PostToServerEng extends AsyncTask<String, String, String>{
		...
		protected void onPostExecute(String result) {
			try {
				...
				Toast.makeText(CaptureSignature.this	,retrunMsg ,Toast.LENGTH_LONG).show();
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		...
		protected String doInBackground(String... params) {
			...
			try {
				...
				if (success==1) {
					ArrayList<EngineerDataBean> beanList=VarList.JOB_BEAN.getEngBean();
					...
					//--------------------------------------------------
					ArrayList<PartsBean> partbean=VarList.JOB_BEAN.getPartBean();
					...
					//-----------------------------------------------
					json = userFunction.createPDF( ID,VarList.JOB_BEAN.getEngineerUser());
					...
					if (successPDF==1) { 
						...
						if (VarList.SELETCED_BTN.equals(ConstList.UNFINISHED_JOBCARD)) {
							...
							JobCardDBAccess dbClass=new JobCardDBAccess(CaptureSignature.this);
		...
		protected void onProgressUpdate(String... values) {
			...
			Toast.makeText(CaptureSignature.this	,values[0] ,Toast.LENGTH_LONG).show();
	...
	private EditText engNameTxt,engStartDateTxt,engFinishDate,EngWorkTime,engTravelTime;
	private EditText customerTXT,visitReasonTXT,serviceTXT,fileamentCounterTXT,opendDateTxt,hvCounterTXT,logTimeTXT,downtimeTXT,jobcardNoTxt,equicmentTXT;
	...
	public void onClick(View v) {
		...
		case R.id.viewParts: 
			...
			try {
				...
				partNameTxt=(EditText) partDailog.findViewById(R.id.viewPartNameTxt);
				partQuantityTxt=(EditText) partDailog.findViewById(R.id.viewPartQuantityTxt);
				partIDTxt=(EditText) partDailog.findViewById(R.id.viewPartIDTxt);
				...
				Button cancelBtnEQ=(Button)partDailog.findViewById(R.id.viewPartCloaseBtn);
				...
				Button addNextBtn=(Button)partDailog.findViewById(R.id.viewPartAddNextBtn);
				addNextBtn.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						...
						if (SIZE>index) {
							...
							partNameTxt.setText(beanList.get(index).getPartName());
							partQuantityTxt.setText(beanList.get(index).getPartQuantitys());
							partIDTxt.setText(beanList.get(index).getPartID());

							index++;
						}else if (SIZE==index) {
							Toast.makeText(getApplicationContext(), "No Parts Available", Toast.LENGTH_LONG).show();
						}
		...
		case R.id.viewEng: 
			...
			try {
				...
				engNameTxt=(EditText) engDailog.findViewById(R.id.viewEngName);
				engStartDateTxt=(EditText) engDailog.findViewById(R.id.viewEngStartDate);
				engFinishDate=(EditText) engDailog.findViewById(R.id.viewEngFinishDate);
				EngWorkTime=(EditText) engDailog.findViewById(R.id.viewTimeworkedEng);
				engTravelTime=(EditText) engDailog.findViewById(R.id.viewTravelTimeEng);
				...
				Button cancelBtnEQ=(Button)engDailog.findViewById(R.id.viewEngClosebtn);
				cancelBtnEQ.setOnClickListener(new OnClickListener() {
				...
				Button addNextBtn=(Button)engDailog.findViewById(R.id.viewengineerBtn);
				addNextBtn.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						...
						if (SIZE>index) {
							...
							engNameTxt.setText(engBeanList.get(index).getEngName());
							engStartDateTxt.setText(engBeanList.get(index).getJobStart());
							engFinishDate.setText(engBeanList.get(index).getJobFinished());
							EngWorkTime.setText(engBeanList.get(index).getWorkedHours());
							engTravelTime.setText(engBeanList.get(index).getTravelTime());

							index++;
						}else if (SIZE==index) {
							Toast.makeText(getApplicationContext(), "No Engineer Available", Toast.LENGTH_LONG).show();
						}
		...
		case R.id.viewJobcard:
			...
			try {
				...
				opendDateTxt=(EditText) jobcardDailog.findViewById(R.id.viewopendate);
				jobcardNoTxt=(EditText) jobcardDailog.findViewById(R.id.viewjobcardNo);
				customerTXT=(EditText) jobcardDailog.findViewById(R.id.viewcustomertxt);
				equicmentTXT=(EditText) jobcardDailog.findViewById(R.id.viewequpmetnTypetxt);
				visitReasonTXT=(EditText) jobcardDailog.findViewById(R.id.viewvisitReason);


				serviceTXT=(EditText) jobcardDailog.findViewById(R.id.viewservicetxt);
				fileamentCounterTXT=(EditText) jobcardDailog.findViewById(R.id.viewfileamentCounterTxt);
				hvCounterTXT=(EditText) jobcardDailog.findViewById(R.id.viewhvCounterTxt);
				logTimeTXT=(EditText) jobcardDailog.findViewById(R.id.viewlogTimeTxt);
				downtimeTXT=(EditText) jobcardDailog.findViewById(R.id.viewdowntimeTxt);
				...
				fileamentCounterTXT.setText(VarList.JOB_BEAN.getFilamnetCounter());
				...
				Button cancelBtnEQ=(Button)jobcardDailog.findViewById(R.id.viewJobcardCloseBtn);
				cancelBtnEQ.setOnClickListener(new OnClickListener() {
		...
		case R.id.visualChck: 
			...
			if (((CheckBox) v).isChecked()) {
				Toast.makeText(CaptureSignature.this,
						"visual check", Toast.LENGTH_LONG).show();
		...
		case R.id.inspectionChck: 
			...
			if (((CheckBox) v).isChecked()) {
				Toast.makeText(CaptureSignature.this,
						"inspection check", Toast.LENGTH_LONG).show();
